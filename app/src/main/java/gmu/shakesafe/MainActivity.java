...
//    public class AnalyzeSensorData extends AsyncTask<SensorEvent, Void, Void> {
//
//        @Override
...
//            // This will avoid slowing down the system when the app is running in the background
//            // and the phone is being used.
//            if (mViewPager.getCurrentItem() == 2 || !isScreenOn) {
...
////                accSensor.setDeltaX((Math.abs(0 - event.values[0])));
////                accSensor.setDeltaY((Math.abs(0 - event.values[1])));
////                accSensor.setDeltaZ((Math.abs(0 - event.values[2])));
...
//                            storeData(0, 0, 0);
//
//                            new UploadData().execute(getApplicationContext());
...
//
//
//                // Local calculations are done. Store values back into object for later use.
